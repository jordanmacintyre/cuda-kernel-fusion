...
try:
    ...
    baseline_func = quantize_int8_pytorch  # Raw PyTorch
    compiled_func = torch.compile(quantize_int8_pytorch)  # torch.compile
    optimized_func = quantize_int8_cuda  # Custom CUDA
...
def run_benchmark_for_size(size, size_name):
    ...
    for _ in range(20):
        _ = compiled_func(x, scale, zero_point)
...
def main():
    ...
    for idx, (size, memory_location) in enumerate(scenarios):
        ...
        result["data_size_mb"] = (size * 4) / (1024 ** 2)
